...
**Total wall-clock for one full 9-stage pipeline run:** ~22 minutes
**Headline model:** Two-Stream RGB+FFT with per-branch Transformer fusion
**Headline test result:** 98.87 % accuracy, 99.19 % recall, 0.9984 ROC-AUC

> **Important:** The numbers below from sections 3–6 are computed on the
> *image-level* 60/20/20 split inherited from the original pipeline. That
> split was later shown by the leakage audit (`research/01`) to contain
> 65 % source-template overlap and 99 % near-duplicate rate, so its absolute
> numbers should be read as a *baseline* rather than a generalisation
> measurement. The headline numbers in **Section 7** below are computed on
> the **template-aware** 60/20/20 split that eliminates the leakage. Full
> details in `RESEARCH_REPORT.md`.
...
## 7. Headline architecture: Two-Stream RGB + FFT + Transformer fusion

After the data-leakage audit (`research/01`) revealed substantial
template overlap in the original image-level split, we re-trained both
the supervised ResNet50 and a novel **Two-Stream RGB+FFT model with
per-branch Transformer fusion** on the **template-aware 60/20/20 split**
that guarantees zero template overlap between splits.

### 7.1 Architecture (`src/two_stream_model.py`)
...
Input (B, 3, 224, 224)
       ...
       │    2-layer Transformer (4 heads, GELU, pre-norm)
       │    take CLS                  →  (B, 256)
       │
       ├─── FFT stream
...
**38.7 M trainable parameters** total. Trained with the same recipe as
the ResNet50 baseline (AdamW lr=1e-4, cosine 15 epochs, AMP, batch=16,
gradient clipping 1.0, class-balanced binary CE).

### 7.2 Headline TEST-set numbers (template-aware, no leakage)

| Method | Acc | F1 | Precision | Recall | ROC-AUC | PR-AUC |
|---|---:|---:|---:|---:|---:|---:|
| Single-stream ResNet50 (clean split) | 0.9639 | 0.9667 | 1.0000 | 0.9355 | 0.9981 | 0.9986 |
| **Two-Stream RGB+FFT + Transformer (this work)** | **0.9887** | **0.9899** | 0.9880 | **0.9919** | **0.9984** | **0.9986** |

Both models effectively saturate ROC-AUC near 0.998. The decisive gain is in
**recall**: the Two-Stream model misses only **2 of 248** test forgeries
(99.19 % recall) versus 16 missed by the single-stream baseline (93.55 %).
For a fraud-detection deployment this is the correct trade — every
additional caught forgery is what justifies the system, while the +1.4 %
precision cost (3 false positives in 195 reals) is acceptable.

### 7.3 Why two-stream works on this dataset

1. **Inpainting and crop-paste leave high-frequency artefacts** that are
   barely visible in RGB but stand out in `log|FFT|`. The FFT branch
   directly attends to those residuals.
2. **Per-branch Transformer over the 7×7 spatial token grid** allows
   localised reasoning ("which patch looks suspicious?") instead of
   averaging into a single 2048-d vector.
3. **Late fusion at the CLS-token level** keeps the two feature
   ...
   FFT features hurt convergence.
4. **Smaller backbone for FFT (ResNet18)** is appropriate because the
   frequency-domain image has much less semantic content than RGB.

Compute: 38.7 M params, peak VRAM 1.47 GB, 7.9 min training time on RTX 2050.
...
The headline result of this work is a **Two-Stream RGB + FFT network
with per-branch Transformer fusion** that achieves **98.87 % test
accuracy, 99.19 % recall, and 0.9984 ROC-AUC** on a template-aware
60 / 20 / 20 split that contains zero template overlap between train
and test (in contrast to the original image-level split, which had
65 % overlap and inflated the apparent baseline ROC-AUC).

The architecture's gain over a strong single-stream ResNet50 baseline
(0.964 → 0.989 accuracy, 93.6 % → 99.2 % recall) is concentrated where
fraud-detection deployments actually need it: **recall**. The two-stream
model misses only 2 forgeries out of 248 in the held-out test set,
versus 16 missed by the single-stream baseline.

The frequency-domain branch is doing exactly what its design predicted:
detecting the high-frequency residual artefacts left by inpainting and
crop-paste forgeries that the RGB branch struggles with on its own.
The per-branch Transformer over the 7×7 spatial token grid enables
localised reasoning that pure average pooling discards.

The full thesis story — including the data-leakage audit, the recovery
on a clean split, and the four diagnostic angles (calibration, failure
analysis, robustness sweep, embedding geometry) — is documented in
`RESEARCH_REPORT.md`.

### Future work

1. **Cross-domain generalisation:** evaluate the trained model on a
   second document family (e.g. MIDV-Holo) to characterise transfer.
2. **Adversarial robustness:** test against deliberately-crafted
   adversarial perturbations (PGD, FGSM, patch attacks).
3. **k-fold cross-validation by template** rather than the single
   60/20/20 split, to obtain confidence intervals on the metrics.
4. **SimCLR pre-train → Two-Stream fine-tune:** use the SimCLR encoder
   as an initialisation for the RGB branch of the two-stream model,
   which typically yields an additional 1–2 % gain in low-data regimes.
